...
def test_bus_value_unsigned() -> None:
...
def test_bus_value_signed() -> None:
    value = BusValue(np.int16(-5))
    assert value.signed_value() == -5
    assert value.is_negative()


def test_bus_value_bit_length() -> None:
    value = BusValue(np.uint16(10))
    assert value.bit_length() == 8


def test_bus_value_get_slice() -> None:
    value = BusValue(np.uint16(0b11110000))
    sliced = value.get_slice(np.uint16(4), np.uint16(8))
    assert sliced.unsigned_value() == 0b1111


def test_bus_value_addition() -> None:
...
def test_bus_value_subtraction() -> None:
...
def test_bus_value_bitwise_operations() -> None:
